...
export default function ProductoKananPage() {
  ...
  const products = useMemo(() => {
    const data = t('brand_pages.kanan.variants', { returnObjects: true }) as VariantData[];
    return data.map((d, i) => ({ ...d, ...(VARIANT_PRESENTATION[i] ?? VARIANT_PRESENTATION[0]) }));
  }, [t]);

  const pills = t('brand_pages.kanan.pills', { returnObjects: true }) as string[];
  ...
  return (
    <>
      <Helmet>
        <title>{t('brand_pages.kanan.meta_title')}</title>
        <meta name="description" content={t('brand_pages.kanan.meta_desc')} />
        <meta name="keywords" content={t('brand_pages.kanan.meta_keywords')} />
        <link rel="canonical" href="https://www.palenkke.org/productos/kanan" />
        <meta property="og:title" content={t('brand_pages.kanan.meta_title')} />
        ...
        <div className="relative z-10 container mx-auto px-6 lg:px-10 pt-40 pb-24 w-full">
          <motion.div variants={stagger} initial="hidden" animate="visible" className="max-w-3xl">
            ...
            <motion.div variants={fadeUp} className="flex items-center gap-3 mb-6">
              ...
              <span className="text-[#C9A84C] text-xs font-semibold tracking-[0.3em] uppercase">{t('brand_pages.kanan.eyebrow')}</span>
            </motion.div>
            ...
            <motion.div
              ...
              <div className="overflow-hidden rounded-sm shadow-[0_24px_80px_rgba(27,58,107,0.12)] group">
                <motion.img
                  ...
                  alt={t('brand_pages.kanan.img_alt')}
              ...
              <motion.div
                ...
                <span className="block font-heading text-2xl font-bold">{t('brand_pages.kanan.origin_badge')}</span>
                <span className="block text-[#0D1B2E]/70 text-xs mt-0.5">{t('brand_pages.kanan.origin_sub')}</span>
              </motion.div>
            </motion.div>
          </div>
        </div>
        ...
        <div className="container mx-auto px-6 lg:px-10">
          <InView className="mb-14">
            ...
            <motion.p variants={fadeUp} className="text-[#5A7099] text-lg mt-3 max-w-xl">
              {t('brand_pages.kanan.variants_sub')}
            </motion.p>
          </InView>
            ...
            <motion.div variants={stagger} className="flex flex-col sm:flex-row gap-4 justify-center">
              <motion.div variants={fadeUp}>
                <Link to="/contacto"
                  ...
                  {t('brand_pages.kanan.cta_button')} <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
                </Link>
              </motion.div>
              <motion.div variants={fadeUp}>
                <Link to="/productos"
